...
type TabType = 'remix' | 'stems' | 'training' | 'history' | 'settings';
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            switch (e.key.toLowerCase()) {
                case ' ':
                    e.preventDefault();
                    if (remixUrl) {
                        if (remixAudioRef.current?.paused) {
                    ...
                    break;
                case 'r':
                    if (audioFile && processingStep === 'idle') {
                        handleCreateRemix();
                    }
                    break;
                case 's':
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 text-white">
            ...
            <main className="max-w-7xl mx-auto px-4 py-6 pb-20">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            >
                                ...
                                <div className="text-center">
                                    ...
                                    <h3 className="text-xl font-semibold mb-2">
                                        {audioFile ? audioFile.name : TEXTS.upload.title}
                                    </h3>
                            ...
                            {remixUrl && (
                                <div className="flex gap-2 flex-wrap">
                                    <button onClick={() => handleExport('mp3')} className="px-4 py-2 bg-green-600 hover:bg-green-500 rounded-lg transition">
                                        {TEXTS.audio.exportMp3}
                                    </button>
                                    <button onClick={() => handleExport('wav')} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg transition">
                                        {TEXTS.audio.exportWav}
                                    </button>
                                    <button onClick={handleExportStems} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 rounded-lg transition">
                                        {TEXTS.audio.exportStems}
                                    </button>
                                    <button
                                        onClick={() => setCompareMode(!compareMode)}
                                        className={`px-4 py-2 rounded-lg transition ${compareMode ? 'bg-yellow-600' : 'bg-gray-600 hover:bg-gray-500'}`}
                                    >
                                        {TEXTS.audio.compare}
                                    </button>
                                </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-black/30 rounded-xl p-4 border border-white/10">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <button
                                        onClick={savePreset}
                                        className="text-xs px-2 py-1 bg-purple-600/30 hover:bg-purple-600/50 rounded transition"
                                    >
                                        {TEXTS.presets.save}
                                    </button>
                                </div>
                                <div className="relative">
                                    <button
                                        ...
                                        className="w-full px-3 py-2 bg-gray-800 rounded-lg border border-white/10 text-left flex items-center justify-between"
                                    >
                                        ...
                                        <div className="absolute top-full left-0 right-0 mt-1 bg-gray-800 rounded-lg border border-white/10 overflow-hidden z-10">
                                            {presets.map(preset => (
                                                <button
                                                    ...
                                                    className={`w-full px-3 py-2 text-left hover:bg-purple-600/30 transition ${preset.id === activePreset ? 'bg-purple-600/50' : ''}`}
                                                >
                                                    {preset.name}
                                                </button>
                                            ))}
                            ...
                            {analysis && (
                                <div className="bg-black/30 rounded-xl p-4 border border-white/10">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-sm">
                                        <span className="text-gray-400">{TEXTS.analysis.bpm}</span>
                                        <span className="font-mono">{analysis.bpm}</span>
                                        <span className="text-gray-400">{TEXTS.analysis.key}</span>
                                        <span>{analysis.key}</span>
                                        <span className="text-gray-400">{TEXTS.analysis.energy}</span>
                                        <span>{Math.round(analysis.energy * 100)}%</span>
                                        <span className="text-gray-400">{TEXTS.analysis.genre}</span>
                            ...
                            <div className="bg-black/30 rounded-xl p-4 border border-white/10 space-y-3">
                                ...
                                <div>
                                    ...
                                    <select
                                        value={settings.targetGenre}
                                        onChange={e => setSettings(s => ({ ...s, targetGenre: e.target.value }))}
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => setSettings(s => ({ ...s, targetBpm: parseInt(e.target.value) }))}
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => setSettings(s => ({ ...s, bassBoost: parseInt(e.target.value) }))}
                                        className="w-full accent-purple-500"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        min="0"
                                        max="100"
                                        value={settings.sidechain * 100}
                                        onChange={e => setSettings(s => ({ ...s, sidechain: parseInt(e.target.value) / 100 }))}
                                        className="w-full accent-purple-500"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => setSettings(s => ({ ...s, reverb: parseInt(e.target.value) / 100 }))}
                                        className="w-full accent-purple-500"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <select
                                        value={settings.kickPattern}
                                        onChange={e => setSettings(s => ({ ...s, kickPattern: e.target.value }))}
                                        className="w-full bg-gray-800 rounded-lg px-3 py-2 border border-white/10 text-sm"
                                    >
                                ...
                                <div>
                                    ...
                                    <select
                                        value={settings.vocalEffect}
                                        onChange={e => setSettings(s => ({ ...s, vocalEffect: e.target.value }))}
                                        className="w-full bg-gray-800 rounded-lg px-3 py-2 border border-white/10 text-sm"
                                    >
                                ...
                                <div className="space-y-2 pt-4">
                                    <button
                                        ...
                                        disabled={!audioFile || processingStep !== 'idle'}
                                        className="w-full py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:cursor-not-allowed rounded-lg transition font-semibold"
                                    >
                                        {TEXTS.controls.separateStems}
                                    </button>

                                    <button
                                        onClick={handleCreateRemix}
                                        disabled={!audioFile || processingStep !== 'idle'}
                                        className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 disabled:from-gray-700 disabled:to-gray-700 disabled:cursor-not-allowed rounded-lg transition font-semibold"
                                    >
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-2">
                                <button
                                    onClick={handleSeparateStems}
                                    disabled={!audioFile || processingStep !== 'idle'}
                                    className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:cursor-not-allowed rounded-lg transition"
                                >
                                    {TEXTS.stems.separate}
                                </button>
                                {stems.length > 0 && (
                                    <button
                                ...
                                {stems.map((stem, i) => (
                                    <div key={i} className="bg-black/30 rounded-xl p-4 border border-white/10">
                                        <div className="flex items-center justify-between mb-3">
                                            ...
                                            <div className="flex gap-2">
                                                <button
                                                    className={`px-3 py-1 rounded text-sm ${stem.muted ? 'bg-red-600' : 'bg-gray-700'}`}
                                                    onClick={() => setStems(stems.map((s, idx) => idx === i ? { ...s, muted: !s.muted } : s))}
                                                >
                                                    M
                                                </button>
                                                <button
                                                    className={`px-3 py-1 rounded text-sm ${stem.solo ? 'bg-yellow-600' : 'bg-gray-700'}`}
                                                    onClick={() => setStems(stems.map((s, idx) => idx === i ? { ...s, solo: !s.solo } : s))}
                                                >
                                                    S
                                                </button>
                                            </div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setStems(stems.map((s, idx) => idx === i ? { ...s, volume: parseInt(e.target.value) / 100 } : s))}
                                            className="w-full accent-purple-500"
                                        />
                ...
                {/* TRAINING TAB - Fast Training với LoRA/QLoRA */}
                {activeTab === 'training' && (
                    <FastTrainingPanel />
                )}
                                    ...
                                    <div key={item.id} className="bg-black/30 rounded-xl p-4 border border-white/10 flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-xs text-purple-400 mt-1">
                                                {item.settings.targetGenre} • {item.settings.targetBpm} BPM
                                            </p>
                                        </div>
                                        <div className="flex gap-2">
                                            <button className="px-3 py-1 bg-green-600/20 text-green-400 hover:bg-green-600/30 rounded transition">
                                                {TEXTS.history.download}
                                            </button>
                                            <button
                                                onClick={() => setHistory(prev => prev.filter(r => r.id !== item.id))}
                    ...
                    <div className="max-w-2xl mx-auto space-y-6">
                        ...
                        <div className="bg-black/30 rounded-xl p-4 border border-white/10 space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    placeholder={TEXTS.settings.checkpointPlaceholder}
                                    className="w-full bg-gray-800 rounded-lg px-3 py-2 border border-white/10"
                                />
                            </div>
                        </div>
                        ...
                        <div className="bg-black/30 rounded-xl p-4 border border-white/10">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-sm">
                                <span className="text-gray-400">{TEXTS.settings.apiServer}</span>
                                <span className="text-green-400">{API_BASE}</span>
                                <span className="text-gray-400">{TEXTS.settings.status}</span>
                                <span className="text-green-400">{TEXTS.settings.ready}</span>
                                <span className="text-gray-400">{TEXTS.settings.gpu}</span>
                                <span className={gpuAvailable ? 'text-green-400' : 'text-yellow-400'}>
                                    {gpuAvailable ? TEXTS.settings.gpuAvailable : TEXTS.settings.gpuNotAvailable}
                                </span>
                            </div>
                        </div>
                        ...
                        <button
                            ...
                            className="w-full py-3 bg-red-600/20 text-red-400 border border-red-500/30 hover:bg-red-600/30 rounded-lg transition"
                        >
